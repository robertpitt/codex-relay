...
import { BackendConfig, BackendServicesBaseLive, fromPromise, runBackendEffect } from "../runtime";
...
type StorageEffect<A> = Effect.Effect<A, unknown>;
...
const storageEffect = <Args extends readonly unknown[], A>(
  evaluate: (...args: Args) => Promise<A>
): ((...args: Args) => StorageEffect<A>) => (...args) => fromPromise(() => evaluate(...args));

const makeFileSystemStorage = (): StorageService => ({
  adapter: "filesystem",
  isGitRepository: storageEffect(FileSystemStorage.isGitRepository),
  isInitialized: storageEffect(FileSystemStorage.isRelayInitialized),
  initializeProject: storageEffect(FileSystemStorage.initializeProject),
  getProjectSummary: storageEffect(FileSystemStorage.summarizeProject),
  getProjectConfig: storageEffect(FileSystemStorage.readProjectConfig),
  saveProjectConfig: storageEffect(FileSystemStorage.writeProjectConfig),
  getBoard: storageEffect(FileSystemStorage.readBoard),
  getTickets: (projectPath) =>
    fromPromise(async () => {
      const board = await FileSystemStorage.readBoard(projectPath);
      return board.tickets;
    }),
  getTicket: storageEffect(FileSystemStorage.readTicket),
  putTicket: storageEffect(FileSystemStorage.writeTicket),
  createTicket: storageEffect(FileSystemStorage.createTicket),
  createSubticket: storageEffect(FileSystemStorage.createSubticket),
  linkSubticket: storageEffect(FileSystemStorage.linkSubticket),
  unlinkSubticket: storageEffect(FileSystemStorage.unlinkSubticket),
  createPendingTicketDraft: storageEffect(FileSystemStorage.createPendingTicketDraft),
  applyTicketDraft: storageEffect(FileSystemStorage.applyTicketDraftToTicket),
  failPendingTicketDraft: storageEffect(FileSystemStorage.failPendingTicketDraft),
  blockPendingTicketDraftForClarification: storageEffect(FileSystemStorage.blockPendingTicketDraftForClarification),
  transitionTicketStatus: storageEffect(FileSystemStorage.transitionTicketStatus),
  setTicketQueued: storageEffect(FileSystemStorage.setTicketQueued),
  clearQueuedTicket: storageEffect(FileSystemStorage.clearQueuedTicket),
  listQueuedReadyTickets: storageEffect(FileSystemStorage.listQueuedReadyTickets),
  listTicketReferenceCandidates: storageEffect(FileSystemStorage.listTicketReferenceCandidates),
  saveTicket: storageEffect(FileSystemStorage.saveTicket),
  saveTicketAttachment: storageEffect(FileSystemStorage.saveTicketAttachment),
  moveTicket: storageEffect(FileSystemStorage.moveTicket),
  getClarificationQuestions: storageEffect(FileSystemStorage.readClarificationQuestions),
  createClarificationQuestions: storageEffect(FileSystemStorage.createClarificationQuestions),
  answerClarificationQuestion: storageEffect(FileSystemStorage.answerClarificationQuestion),
  deleteTicket: storageEffect(FileSystemStorage.deleteTicket),
  duplicateTicket: storageEffect(FileSystemStorage.duplicateTicket),
  revealTicketFile: storageEffect(FileSystemStorage.revealTicketFile)
});

const storageForAdapter = (adapter: StorageAdapterName): StorageService => {
  switch (adapter) {
    case "filesystem":
      return makeFileSystemStorage();
  }
};

export const storageLayerForAdapter = (adapter: StorageAdapterName) => Layer.succeed(Storage)(storageForAdapter(adapter));

export const FileSystemStorageLive = storageLayerForAdapter("filesystem");

export const StorageLive = Layer.effect(
  Storage,
  Effect.gen(function*() {
    const config = yield* BackendConfig;
    return storageForAdapter(config.storageAdapter);
  })
);
