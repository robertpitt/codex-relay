...
export type ClarificationStoreService = {
  readonly list: (projectPath: string, ticketId: string) => StoreEffect<ClarificationQuestion[]>;
  readonly create: (
    projectPath: string,
    ticketId: string,
    inputs: readonly ClarificationQuestionCreateInput[],
    options: ClarificationQuestionCreateOptions
  ) => StoreEffect<ClarificationQuestion[]>;
  readonly answer: (projectPath: string, ticketId: string, questionId: string, answer: string) => StoreEffect<ClarificationQuestion>;
};
...
export const makeFileSystemClarificationStore = (): ClarificationStoreService => ({
  list: (projectPath, ticketId) =>
    storeRead(clarificationStorePath(projectPath, ticketId), "Read Relay clarification questions", () =>
      FileSystemStorage.readClarificationQuestions(projectPath, ticketId)
    ),
  create: (projectPath, ticketId, inputs, options) =>
    storeWrite(clarificationStorePath(projectPath, ticketId), "Create Relay clarification questions", () =>
  ...
  answer: (projectPath, ticketId, questionId, answer) =>
    storeWrite(clarificationStorePath(projectPath, ticketId), "Answer Relay clarification question", () =>
